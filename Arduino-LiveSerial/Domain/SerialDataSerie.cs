using Arduino_LiveSerial.View.Dialogs;
using Arduino_LiveSerial.ViewModels;
using Arduino_LiveSerial.Views;
...
namespace Arduino_LiveSerial.Domain
{
    ...
    {
        public ReactiveCommand<Unit, Unit> OpenSerieCommand { get; set; }

        public SerialDataSerie()
        {
            peakSearch = new PeakSearch(this);
            OpenSerieCommand = ReactiveCommand.Create(OpenSerie);
        }

        private void OpenSerie()
        {
            var context = new SerieVisualizerViewModel() { Data = this.Data.ToArray(), SerieName = Name};
            var viewSerieWindow = new ViewSerieWindow()
            {
                Title = $"Serie: {Name}",
                DataContext = context,
            };

            viewSerieWindow.Show();
        }
